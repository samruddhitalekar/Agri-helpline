...
<?php
    if(isset($_POST['change']))
    {
        ...
        $chpass=mysqli_query($connect,"select * from customer where Customer_email='".$_SESSION['customer_email']."' and Customer_password='".$_POST['oldpass']."'") or die(mysqli_error($connect));
        if(mysqli_num_rows($chpass)==1)
        {
            if(strlen($_POST['newpass'])>=3)
            {
                ...
                {
                    $update=mysqli_query($connect,"update customer set Customer_password='".$_POST['newpass']."' , 
                            Customer_password='".$_POST['repass']."' where Customer_email='".$_SESSION['customer_email']."' " )or die(mysqli_error($connect));
                    if($update)
                    {
                        echo "<script>";
